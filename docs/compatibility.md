...
- **Observed route**: the named service was known during testing, but LyricBar only saw a generic MPRIS player and cannot identify or support that service independently.
...
| TIDAL Web                      | Generic Chrome MPRIS    | Observed route | TIDAL was known externally to be playing during the capture, but Chrome exposed only its generic MPRIS identity. Synced lyrics worked for sampled tracks through the existing Chromium path. LyricBar cannot detect TIDAL, and this is not dedicated TIDAL support.                                                                             |
...
5. TIDAL clients that expose a stable service identity, including native, wrapper, Firefox, or other browser routes.
